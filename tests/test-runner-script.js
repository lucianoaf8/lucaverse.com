...
class TestStudio {
    ...
    updatePerformanceDisplay() {
        ...
        this.updateMiniChart('speed-chart', this.performanceMetrics.speed);
        this.updateMiniChart('memory-chart', this.performanceMetrics.memory);
        this.updateMiniChart('error-chart', this.performanceMetrics.errorRate);
    }
    ...
    updateMiniChart(chartId, data) {
        ...
        const percentage = ((latest - min) / range) * 100;
        
        chart.style.background = `linear-gradient(90deg, var(--accent-blue) ${percentage}%, rgba(96, 165, 250, 0.2) ${percentage}%)`;
    }
